...
export default function InvestorsPage() {
  ...
  const { data: partners } = useApi<Partner[]>("/partners/partners/", []);
  ...
  return (
    <div className="min-h-screen bg-[#030303]">
        ...
        <div className="relative mx-auto max-w-7xl text-center">
          ...
          <div className="mx-auto mt-10 grid max-w-3xl grid-cols-2 gap-4 sm:grid-cols-4">
            {[
              ...
              <div key={s.label} className="rounded-2xl border border-white/5 bg-white/2 px-4 py-5">
      ...
      <div className="mx-auto max-w-7xl px-6 py-14">
        {/* Why Uychi */}
        <div className="mb-16">
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-cyan-400">/ Nima uchun Uychi?</p>
          <h2 className="mt-3 text-[clamp(1.5rem,3vw,2.5rem)] font-bold text-white">Investitsiya afzalliklari</h2>
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {PERKS.map((p) => (
              <div key={p.title} className={`rounded-2xl border p-6 transition-all hover:-translate-y-1 ${
                p.accent === "cyan" ? "border-cyan-500/15 bg-cyan-500/3" :
                ...
                <div className={`mb-4 flex h-10 w-10 items-center justify-center rounded-xl border text-lg font-bold ${
                  p.accent === "cyan" ? "border-cyan-500/20 bg-cyan-500/10 text-cyan-400" :
                ...
                <p className="mt-2 text-[12px] leading-relaxed text-zinc-500">{p.desc}</p>
                <p className={`mt-3 text-[11px] font-bold ${
                  p.accent === "cyan" ? "text-cyan-400" :
        ...
        <div className="mb-16">
          ...
          <h2 className="mt-3 text-[clamp(1.5rem,3vw,2.5rem)] font-bold text-white">Asosiy yo'nalishlar</h2>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {OPPORTUNITIES.map((opp) => (
              <div key={opp.title} className={`rounded-2xl border p-6 transition-all hover:-translate-y-0.5 ${
                opp.accent === "cyan" ? "border-cyan-500/15 bg-[#0a0a0a]" :
                opp.accent === "violet" ? "border-violet-400/15 bg-[#0a0a0a]" :
                "border-emerald-400/15 bg-[#0a0a0a]"
              }`}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <span className={`inline-block rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase ${
                      opp.accent === "cyan" ? "border-cyan-500/20 bg-cyan-500/10 text-cyan-400" :
                      opp.accent === "violet" ? "border-violet-400/20 bg-violet-500/10 text-violet-400" :
                      "border-emerald-400/20 bg-emerald-500/10 text-emerald-400"
                    }`}>{opp.type}</span>
                    <h3 className="mt-3 text-[17px] font-bold text-white">{opp.title}</h3>
                    <p className="mt-1.5 text-[13px] leading-relaxed text-zinc-500">{opp.desc}</p>
                  </div>
                </div>
                <div className="mt-5 flex items-center justify-between border-t border-white/5 pt-4">
                  <div>
                    <p className="text-[11px] text-zinc-600">Investitsiya hajmi</p>
                    <p className={`text-[16px] font-bold ${
                      opp.accent === "cyan" ? "text-cyan-400" :
                      opp.accent === "violet" ? "text-violet-400" : "text-emerald-400"
                    }`}>{opp.amount}</p>
                  </div>
                  <button className="rounded-xl border border-white/10 bg-white/3 px-4 py-2 text-[12px] font-semibold text-zinc-400 transition-all hover:border-white/20 hover:text-white">
                    Batafsil
                  </button>
                </div>
              </div>
        ...
        <div className="mb-16">
          ...
          <h2 className="mt-3 text-[clamp(1.5rem,3vw,2.5rem)] font-bold text-white">Bizga ishongan tashkilotlar</h2>
          <div className="mt-8 flex flex-wrap gap-3">
            {investors.map((p) => (
              <div key={p.id} className="flex items-center gap-2 rounded-xl border border-white/8 bg-white/3 px-4 py-2.5 text-[13px] font-semibold text-zinc-400">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {/* Download deck */}
          <div className="overflow-hidden rounded-2xl border border-cyan-500/15 bg-gradient-to-br from-[#0a1a1a] to-[#0a0a0a] p-8">
            <p className="text-[11px] font-semibold uppercase tracking-[0.15em] text-cyan-400">Investitsiya Taqdimoti</p>
            <h3 className="mt-2 text-xl font-bold text-white">To'liq Hujjatlarni Yuklab Oling</h3>
            <p className="mt-2 text-[14px] leading-relaxed text-zinc-500">Bozor ma'lumotlari, imtiyozlar tuzilmasi, ROI modellari va tasdiqlangan muvaffaqiyat hikoyalarini o'z ichiga olgan keng qamrovli taqdimot.</p>
            <a href="#" className="mt-6 inline-flex items-center gap-2.5 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-6 py-3 text-[13px] font-bold text-cyan-400 transition-all hover:border-cyan-400/50 hover:bg-cyan-500/15">
          ...
          {/* Contact form */}
          <div className="rounded-2xl border border-white/6 bg-[#0a0a0a] p-8">
            <p className="text-[11px] font-semibold uppercase tracking-[0.15em] text-violet-400">Bog'lanish</p>
            <h3 className="mt-2 text-xl font-bold text-white">Investitsiya Muzokarasi</h3>
            {formSent ? (
              <div className="mt-6 flex flex-col items-center rounded-xl border border-emerald-400/20 bg-emerald-500/5 py-10 text-center">
                ...
                <p className="mt-2 font-bold text-white">Xabaringiz yuborildi!</p>
                <p className="mt-1 text-[13px] text-zinc-500">24 soat ichida javob beramiz.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="mt-5 grid gap-3 sm:grid-cols-2">
                {[
                  ...
                  <div key={f.label} className={f.span === 2 ? "sm:col-span-2" : ""}>
                    <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-zinc-600">{f.label}</label>
                    <input name={f.name} type={f.type || "text"} placeholder={f.placeholder} required className="w-full rounded-xl border border-white/6 bg-white/2 px-4 py-3 text-[13px] text-white outline-none placeholder:text-zinc-700 focus:border-violet-400/30" />
                  </div>
                ))}
                <div className="sm:col-span-2">
                  <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-zinc-600">Xabar</label>
                  <textarea name="message" rows={3} placeholder="Investitsiya yo'nalishi va qo'shimcha ma'lumotlar..." className="w-full resize-none rounded-xl border border-white/6 bg-white/2 px-4 py-3 text-[13px] text-white outline-none placeholder:text-zinc-700 focus:border-violet-400/30" />
                </div>
                <div className="sm:col-span-2">
                  <button type="submit" disabled={sending} className="w-full rounded-full bg-violet-500 py-3.5 text-[14px] font-bold text-white transition-all hover:bg-violet-400 disabled:opacity-60">
