...
const A: Record<AccentKey, { border: string; badge: string; text: string; dot: string; glow: string }> = {
  cyan:    { border: "border-cyan-500/20 hover:border-cyan-500/40",    badge: "bg-cyan-500/10 text-cyan-400 border-cyan-500/20",    text: "text-cyan-400",    dot: "bg-cyan-400",    glow: "hover:shadow-[0_0_30px_-5px_rgba(6,247,227,0.15)]" },
...
const STAGE_COLORS: Record<string, string> = {
  "Idea": "text-zinc-400 border-zinc-500/20 bg-zinc-500/8",
...
export default function StartupsPage() {
  const { data: all, loading } = useApi<Startup[]>("/startups/startup-applications/", []);
    ...
    <div className="min-h-screen bg-[#030303]">
      <section className="relative border-b border-white/4 px-6 py-16">
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <div className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/3 px-4 py-3 sm:w-72">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Startap qidirish..." className="flex-1 bg-transparent text-[14px] text-white outline-none placeholder:text-zinc-600" />
            </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className="mb-8 flex flex-wrap gap-2">
          {STAGES.map(s => (
            <button key={s} onClick={() => setStage(s)} className={`rounded-full border px-3 py-1.5 text-[11px] font-semibold transition-all ${stage === s ? "border-violet-400/40 bg-violet-500/10 text-violet-400" : "border-white/8 bg-white/3 text-zinc-500 hover:text-zinc-300"}`}>{s}</button>
          ))}
          {sectors.map(s => (
            <button key={s} onClick={() => setSector(s)} className={`rounded-full border px-3 py-1.5 text-[11px] font-semibold transition-all ${sector === s ? "border-cyan-500/40 bg-cyan-500/10 text-cyan-400" : "border-white/8 bg-white/3 text-zinc-500 hover:text-zinc-300"}`}>{s}</button>
          ))}
        </div>

        {loading && (
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-64 animate-pulse rounded-2xl border border-white/5 bg-[#0a0a0a]" />)}
          </div>
        )}

        {!loading && filtered.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-center">
            <p className="text-[15px] text-zinc-500">Startap topilmadi</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {filtered.map((s, idx) => {
              ...
              return (
                <div key={s.id} className={`group flex flex-col rounded-2xl border bg-[#0a0a0a] p-6 transition-all duration-300 hover:-translate-y-1 ${c.border} ${c.glow}`}>
                  <div className="flex items-start justify-between">
                    ...
                    <span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${STAGE_COLORS[s.stage] || "text-zinc-400 border-zinc-400/20"}`}>{s.stage}</span>
                  </div>
                  <h3 className="mt-4 text-[16px] font-bold text-white">{s.startup_name}</h3>
                  <p className="mt-2 flex-1 text-[13px] leading-relaxed text-zinc-500">{s.description}</p>
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {techTags.map((t, i) => <span key={i} className="rounded bg-white/4 px-2 py-0.5 font-mono text-[10px] text-zinc-500">{t.trim()}</span>)}
                  </div>
                  <div className="mt-4 grid grid-cols-3 gap-3 border-t border-white/4 pt-4 text-[12px]">
                    <div><span className="text-zinc-600">Asoschisi</span><p className="font-medium text-white">{s.founder_name}</p></div>
                    <div><span className="text-zinc-600">Jamoa</span><p className="font-medium text-white">{s.team_size} kishi</p></div>
                    <div><span className="text-zinc-600">Moliya</span><p className={`font-bold ${c.text}`}>{s.funding_needed || "—"}</p></div>
                  </div>
                </div>
              );
            })}
        ...
        {!loading && (
          <div className="mt-12 grid grid-cols-3 gap-4 rounded-2xl border border-white/5 bg-white/2 p-6 text-center">
            {[
