...
const A: Record<AccentKey, { border: string; badge: string; text: string }> = {
  cyan:    { border: "border-cyan-500/20 hover:border-cyan-500/40",    badge: "bg-cyan-500/10 text-cyan-400 border-cyan-500/20",    text: "text-cyan-400" },
...
const TYPE_COLORS: Record<string, string> = {
  "Full-time": "text-cyan-400 border-cyan-500/20 bg-cyan-500/8",
...
export default function JobsPage() {
  const { data: allJobs, loading } = useApi<Job[]>("/careers/job-postings/", []);
      ...
      <section className="relative border-b border-white/4 px-6 py-16">
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <p className="mt-4 max-w-xl text-[15px] leading-relaxed text-zinc-500">
            Uychi AI & IT Hub taklif etayotgan ish o&apos;rinlari va stajyorlik dasturlari.
          </p>
          <div className="mt-8 flex flex-wrap gap-4">
            {[
              { label: "Ochiq vakansiyalar", value: loading ? "—" : String(activeJobs.length), color: "text-emerald-400" },
              { label: "Bo'limlar", value: loading ? "—" : String(new Set(activeJobs.map(j => j.department)).size), color: "text-cyan-400" },
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center">
          <div className="flex flex-1 items-center gap-3 rounded-2xl border border-white/8 bg-white/3 px-4 py-3">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Lavozim yoki bo'lim qidirish..." className="flex-1 bg-transparent text-[14px] text-white outline-none placeholder:text-zinc-600" />
          </div>
          <div className="flex flex-wrap gap-2">
            {FILTER_TYPES.map((f) => (
              <button key={f.key} onClick={() => setTypeFilter(f.key)} className={`rounded-full border px-3 py-1.5 text-[11px] font-semibold transition-all ${typeFilter === f.key ? "border-emerald-400/40 bg-emerald-500/10 text-emerald-400" : "border-white/8 bg-white/3 text-zinc-500 hover:text-zinc-300"}`}>{f.label}</button>
            ))}
          </div>
        </div>
        ...
        {!loading && filtered.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-center">
            <p className="text-[15px] text-zinc-500">Vakansiya topilmadi</p>
          ...
          <div className="space-y-4">
            {filtered.map((job, idx) => {
              ...
              const typeColor = TYPE_COLORS[job.employment_type] || "text-zinc-400 border-zinc-400/20 bg-zinc-500/8";
              return (
                <div key={job.id} className={`group flex flex-col rounded-2xl border bg-[#0a0a0a] p-6 transition-all duration-200 hover:-translate-y-0.5 sm:flex-row sm:items-center sm:gap-6 ${c.border}`}>
                  ...
                  <div className="mt-4 flex-1 sm:mt-0">
                    ...
                    <p className="mt-1 text-[13px] text-zinc-500">{job.department} · {job.location}</p>
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {job.requirements.split(",").slice(0, 4).map((req, i) => (
                        <span key={i} className="rounded bg-white/4 px-2 py-0.5 font-mono text-[10px] font-medium text-zinc-500">{req.trim()}</span>
                      ))}
                    </div>
                  </div>
                  <div className="mt-4 flex items-center justify-between gap-4 sm:mt-0 sm:flex-col sm:items-end">
                    <div className="text-right">
                      ...
                      <p className="mt-0.5 text-[11px] font-medium text-zinc-600">{job.applicants_count} ariza</p>
                    </div>
        ...
        <div className="mt-16 rounded-2xl border border-emerald-400/15 bg-gradient-to-br from-[#0a1a0f] to-[#0a0a0a] p-8 text-center">
          ...
          <h3 className="mt-2 text-2xl font-bold text-white">Mos Vakansiya Kutib Turayotganlar Uchun</h3>
          <p className="mx-auto mt-3 max-w-lg text-[14px] leading-relaxed text-zinc-500">Rezyumeyingizni yuklang — HR jamoamiz sizga mos vakansiya paydo bo&apos;lganda 24 soat ichida xabar beradi.</p>
